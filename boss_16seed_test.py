...
matchupstats_original_26 = matchupstats_original_26[
    matchupstats_original_26["round"] == ROUND_OF_64
].copy()
matchupstats_original_26["upset_int"] = matchupstats_original_26["is_upset"].astype(int)

top15_by_year_26 = pd.read_csv(TOP15_PATH)
...
for target_year in all_years:
    ...
    treatment = matchupstats_original_26[
        (matchupstats_original_26["season"].isin(prior_years)) &
        (matchupstats_original_26["upset_int"] == 1)
    ]
    ...
    control_pool = matchupstats_original_26[
        (matchupstats_original_26["season"] == target_year) &
        (
            matchupstats_original_26["lteam_seed"].isin(UPSET_SEEDS) |
            matchupstats_original_26["wteam_seed"].isin(UPSET_SEEDS)
        )
    ]
    ...
    year_top15 = top15_by_year_26[top15_by_year_26["target_year"] == target_year]
    if len(year_top15) == 0:
    ...
    for tau in TAU_VALUES:
        ...
        val_upsets = set(
            matchupstats_original_26[
                (matchupstats_original_26["season"] == val_year) &
                (matchupstats_original_26["upset_int"] == 1)
            ].index
        )
    ...
    for rank, idx in enumerate(top2_final, start=1):
        row = matchupstats_original_26.loc[idx]
