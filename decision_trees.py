...
matchupstats_original = matchupstats_original[matchupstats_original["round"] == ROUND_OF_64].copy()
...
matchup_cols = [c for c in matchupstats_original.columns if c.startswith("matchup_")]
print(f"Features found: {len(matchup_cols)}")
print(f"Total games after filter: {len(matchupstats_original)}")
print(f"Upsets: {matchupstats_original['upset_int'].sum()}  |  Non-upsets: {(matchupstats_original['upset_int'] == 0).sum()}")
print()

# leave-one-year-out feature importance

years = sorted(matchupstats_original["season"].unique())
...
for target_year in target_years:
    train = matchupstats_original[matchupstats_original["season"] < target_year]
...
# Save results
out_path = Path(__file__).parent / "top15_by_year.csv"
rows = []
